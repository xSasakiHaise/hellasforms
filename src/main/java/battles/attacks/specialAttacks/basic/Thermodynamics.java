...
public class Thermodynamics extends SpecialAttackBase {
    public void applyAfterEffect(PixelmonWrapper pw) {
        if (pw.getSpecies().is(new RegistryValue[]{PixelmonSpecies.HOOPA})) {
            if (pw.getForm().isForm(new String[]{"hellas"})) {
                pw.setForm("unleashed");
            } else if (pw.getForm().isForm(new String[]{"unleashed"})) {
                pw.setForm("hellas");
            }
